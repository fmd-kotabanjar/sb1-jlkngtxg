...
-- Comprehensive policy cleanup function
DO $$ 
DECLARE
...
BEGIN
    ...
    FOR policy_record IN 
        ...
        FROM pg_policies 
        WHERE tablename IN ('profiles', 'prompts', 'user_claimed_prompts', 'user_roles', 'prompt_requests', 'redeem_codes')
...
CREATE POLICY "user_claimed_prompts_select_own"
  ON user_claimed_prompts
  FOR SELECT
  TO public
  USING (auth.uid() = user_id);

CREATE POLICY "user_claimed_prompts_insert_own"
  ON user_claimed_prompts
  FOR INSERT
  TO public
...
CREATE POLICY "user_roles_first_admin"
  ON user_roles
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "prompt_requests_select_own"
  ON prompt_requests
  FOR SELECT
  TO public
  USING (auth.uid() = user_id);

CREATE POLICY "prompt_requests_insert_limited"
  ON prompt_requests
  FOR INSERT
  TO public
  WITH CHECK (
...
CREATE POLICY "redeem_codes_select_available"
  ON redeem_codes
  FOR SELECT
  TO public
